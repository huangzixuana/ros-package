...
@author: zcx
'''
class FinetunePointSM(Behavior):
	'''
	finetune
	'''
	...
	def __init__(self):
		super(FinetunePointSM, self).__init__()
		self.name = 'FinetunePoint'
	...
	def create(self):
		...
		with _state_machine:
			...
			OperatableStateMachine.add('startAdjust',
										PublishHeader(seq=self.yline_gap, frame_id="enable_hole"),
			...
			OperatableStateMachine.add('info',
										LineAdjust(adjust_topic="/detected/move_xy_rz", timeout=60, fresh_time=3.0, ideal=[0, 0, 0, 0, 0, 0], tolerance=[-1, -1, -1, -1, -1, -1], max_z=0.15),
